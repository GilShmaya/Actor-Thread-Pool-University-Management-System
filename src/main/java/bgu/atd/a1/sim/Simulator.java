...
public class Simulator {
	...
	*/
	...
	private static void submitAction(Input.ActionArgs actionArgs){
		...
		Action<Pair<Boolean,String>> action;
		String actorName;
		PrivateState actorState;
		SchedualPhase schedualPhase;
		switch (actionName){
			case "Open Course":
				actorName=
						actorState=
				action=new OpenANewCourseAction(actionName, actionArgs.departmentName, actionArgs.courseName, actionArgs.space, actionArgs.prerequisites), actionArgs.departmentName

				actorThreadPool.submit(new SchedualPhase(),actionArgs.departmentName, actorThreadPool.getPrivateState(actionArgs.departmentName) == null ? actorThreadPool.getPrivateState(actionArgs.departmentName) : new DepartmentPrivateState());
			case "Add Student":
				actorThreadPool.submit(new AddStudentAction(actionName, actionArgs.departmentName, actionArgs.studentId),actionArgs.departmentName,actorThreadPool.getPrivateState(actionArgs.departmentName) == null ? actorThreadPool.getPrivateState(actionArgs.departmentName) : new DepartmentPrivateState());
			...
			case "Close Course":
				actorThreadPool.submit(new CloseACourseAction(actionName,actionArgs.departmentName, actionArgs.courseName),actionArgs.departmentName,actorThreadPool.getPrivateState(actionArgs.departmentName) == null ? actorThreadPool.getPrivateState(actionArgs.departmentName) : new DepartmentPrivateState());
			case "Add Spaces":
